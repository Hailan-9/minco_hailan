 ...
 * @LastEditors: Shirley
 * @LastEditTime: 2022-10-08 22:11:34
...
namespace lbfgs
{
    ...
    struct lbfgs_parameter_t
    {
        ...
        // Shirley variables
        bool use_alm;
        double alm_delta, conic_crit, equal_crit, inequal_crit;
        Eigen::Matrix3Xd *mu_rho;
        Eigen::Matrix3Xd *projection_on_symm_cone;
        Eigen::VectorXd *equal_criterion;
        Eigen::VectorXd *inequal_criterion;
    };
     ...
     */
    // 这是一个函数指针类型(注意是类型类型类型)的定义 double是指这个函数指针所指向的函数应该返回一个double类型的值
    // (*lbfgs_evaluate_t) 指明这是一个指向函数的指针
    // (void *instance, const Eigen::VectorXd &x, Eigen::VectorXd &g) 
    // 是这个函数指针所指向函数的参数列表
    // 这个函数指针类型的定义 lbfgs_evaluate_t 表示它可以指向一个函数，这个函数接受三个参数，
    // 并返回一个 double 类型的值。
    // 通常，这种技术可以用于传递函数作为参数，或者在某些库或框架中用于回调函数。
     ...
     *
     *
     *  @param  x               The vector of decision variables.决策变量构成的向量！！！
     ...
     * 给定变量x的当前值，提供函数f（x）和梯度g（x）求值的回调函数。
     * 客户端程序必须实现与lbfgs_evaluate_t兼容的回调函数，并将指针传递给回调函数。
     ...
     */
    // 优化算法函数，函数参数中有三个函数指针，很妙很高级的用法。用到了函数指针类型！！
            ...
            */
            ...
            while (true)
            {
                ...
                */
                ...
                // Shirley: Stopping criterion test for  ALM.
                if (param.use_alm)
                {
                    auto equal_criterion_ptr = param.equal_criterion;
                    auto inequal_criterion_ptr = param.inequal_criterion;
                    auto mu_rho_ptr = param.mu_rho;
                    auto projection_on_symm_cone_ptr = param.projection_on_symm_cone;
                    double equal_crit = (*equal_criterion_ptr).cwiseAbs().maxCoeff();
                    double inequal_crit = (*inequal_criterion_ptr).cwiseAbs().maxCoeff();
                    double conic_crit = (*mu_rho_ptr - *projection_on_symm_cone_ptr).cwiseAbs().maxCoeff();
                    param.conic_crit = conic_crit;
                    param.equal_crit = equal_crit;
                    param.inequal_crit = inequal_crit;
                    if (gnorm_inf <= param.alm_delta * std::min(1.0, std::max(inequal_crit, std::max(conic_crit, equal_crit))))
                    {
                        ret = LBFGS_STOP;
                        break;
                    }
